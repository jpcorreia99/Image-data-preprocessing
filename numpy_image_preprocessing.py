...
def process_images_npy(original_data_path,    # directory where the images are originally stored
                       ...
                       grayscale = False): # convert the image to black and white?
    ...
    for class_name in class_list:
        ...
        for img_name in os.listdir(class_path):
            ...
            image_data = image_data.resize((image_resolution[0], image_resolution[1])) #resizing the image

            if grayscale: #whether to convert to grayscale
                image_data = image_data.convert('L')

            image_data = np.asarray(image_data)
            class_data_container.append(image_data)
